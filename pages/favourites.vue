    ...
    <q-layout class="rubik">
        ...
        <q-page class="q-pa-md" style="width: 100%;">
            ...
            <div class="filterSection">
                <div class="types">
                    <q-btn-toggle class="toggle" no-caps v-model="sellType" flat :options="[
                ...
                <div class="search">
                    <q-input dense outlined v-model="searchText" label="Поиск" class="searchField">
            ...
            <div style="width: 100%;" v-for="card in filteredProducts">
               <productCardCe :image="card.image" :type="card.type" :name="card.name" :city="card.city" :seller="card.seller" :category="card.category" :description="card.description" :finalPrice="card.price*card.quantity" :quantity="card.quantity" :price="card.price" :obj="card" :currentRoute="route.path"/> 
            </div>
            <div style="color: #2D3B87; font-size: 15px; margin-top: 20px;" v-if="filteredProducts[0] == undefined">Результатов: 0</div>
        </q-page>
      </q-page-container>
...
<script setup lang="ts">
    ...
    const deals = store.$state.favorites;
    ...
    return deals.filter(product => {
        ...
        const matchesType = sellType.value === 'all' || product.typeEN.toLowerCase() === sellType.value.toLowerCase();
